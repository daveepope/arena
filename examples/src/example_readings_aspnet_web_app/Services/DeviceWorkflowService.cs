...
public interface IDeviceWorkflowService
{
    ...
    Task<DeviceStateResponse?> RequestTransitionAsync(int deviceId, string target);
...
public class DeviceWorkflowService : IDeviceWorkflowService
{
    ...
    public async Task<DeviceStateResponse?> RequestTransitionAsync(int deviceId, string target)
    {
        var workflowId = $"device-{deviceId}";
        try
        {
            ...
            var result = await handle.ExecuteUpdateAsync(
                (DeviceLifecycleWorkflow wf) => wf.RequestTransition(target));
