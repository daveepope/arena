...
@pytest.mark.asyncio
async def test_fastapi_readings_happy_path_sqs_event_and_list(
    readings_fastapi_ctx,
    arena,
    ...
    @playbook(validation_db_playbook, ctx.localstack_session_playbook)
    async def _body(arena):
        sqs = boto3.client(
            "sqs",
