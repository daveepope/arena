...
- **Rust: no inline `#[cfg(test)] mod tests` blocks in `src/`.** Split each crate's `tests/` directory into `tests/unit/` and `tests/component/` subfolders. `tests/unit/` mirrors the crate's `src/` module structure 1:1 (e.g. `src/oracle_dependency/healthcheck.rs` → `tests/unit/oracle_dependency/healthcheck_test.rs`); `tests/component/` holds the container-backed `component_test.rs`. This is the standard going forward for new crates. Functions under test must be `pub` (or `pub(crate)` re-exported as needed) so the external test file can call them, and each test file needs a matching `rust_test` target in `BUILD` — this repo is Bazel-first, so `bazel test` (not plain `cargo test` auto-discovery) is the authoritative way to run tests nested under `tests/unit/`/`tests/component/`.
...
- Tests that **start a running dependency** (container or in-process server) must declare **`tags = ["component_test"]`** on their **`rust_test`** / **`py_test`** / **`java_test`** target. PR CI runs the full suite on **`ubuntu-24.04`** (linux/amd64), on **`ubuntu-24.04-arm`** (linux/arm64, with `qemu-user-static` registered so amd64-only images still run), and on **`windows-latest`**, each with a companion job for `//examples/...`. **`macos-15-intel`** and **`macos-latest`** have no container runtime and run build plus non-`component_test` tests only. Coverage and hosts without a container runtime use **`--test_tag_filters=-component_test`**.
