...
pub extern "C" fn arena_mssql_playbook_open(
    ...
    if arena_handle.is_null() {
        unsafe { write_error(err_out, "arena_mssql_playbook_open: arena handle must not be null") };
    ...
    let spec_str = match unsafe { c_str_to_string(spec) } {
        Some(v) => v,
        None => {
            unsafe { write_error(err_out, "arena_mssql_playbook_open: spec is not valid UTF-8") };
            return std::ptr::null_mut();
        }
    };
    ...
    let parsed: PlaybookSpec = match serde_json::from_str(&spec_str) {
        ...
        Err(e) => {
            unsafe {
                write_error(err_out, format!("arena_mssql_playbook_open: spec parse failed: {e}"))
            };
            return std::ptr::null_mut();
        }
    };
    ...
    let outcome = catch_unwind(AssertUnwindSafe(|| -> Result<PlaybookInner, String> {
        ...
        let playbook = with_mssql_dependency(inner, &parsed.dependency_identifier, |mssql| {
            mssql.playbook()
        })?;
    ...
    match outcome {
        ...
        Ok(Err(msg)) => {
            log::error!("arena_mssql_playbook_open failed: {msg}");
            unsafe { write_error(err_out, format!("arena_mssql_playbook_open: {msg}")) };
            std::ptr::null_mut()
        }
        Err(payload) => {
            let msg = panic_message(&payload);
            log::error!("panic in arena_mssql_playbook_open: {msg}");
            unsafe {
                write_error(err_out, format!("panic in arena_mssql_playbook_open: {msg}"))
            };
            std::ptr::null_mut()
        }
    }
...
pub extern "C" fn arena_mssql_playbook_close(
    ...
    match outcome {
        ...
        Err(payload) => {
            let msg = panic_message(&payload);
            log::error!("arena_mssql_playbook_close: {msg}");
...
pub extern "C" fn arena_mssql_playbook_verify(
    ...
    if handle.is_null() {
        unsafe { write_error(err_out, "arena_mssql_playbook_verify: handle must not be null") };
        return ArenaStatus::InvalidArgument;
    }
    if verify_spec.is_null() {
        unsafe {
            write_error(err_out, "arena_mssql_playbook_verify: verify_spec must not be null")
        };
        return ArenaStatus::InvalidArgument;
    }
    ...
    let spec_str = match unsafe { c_str_to_string(verify_spec) } {
        ...
        None => {
            unsafe {
                write_error(err_out, "arena_mssql_playbook_verify: verify_spec is not valid UTF-8")
            };
            return ArenaStatus::InvalidArgument;
        }
    };
    ...
    let parsed: VerifySpec = match serde_json::from_str(&spec_str) {
        ...
        Err(e) => {
            unsafe {
                write_error(err_out, format!("arena_mssql_playbook_verify: parse failed: {e}"))
            };
            return ArenaStatus::InvalidArgument;
        }
    };
    ...
    match outcome {
        ...
        Err(payload) => {
            let msg = panic_message(&payload);
            log::error!("arena_mssql_playbook_verify failed: {msg}");
            unsafe { write_error(err_out, format!("arena_mssql_playbook_verify: {msg}")) };
            ArenaStatus::Failed
        }
    }
