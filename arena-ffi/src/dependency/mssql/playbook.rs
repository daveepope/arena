...
use arena_mssql::ActivePlaybook;
...
pub extern "C" fn arena_mssql_playbook_verify(
    ...
    let parsed: VerifySpec = match catch_unwind(AssertUnwindSafe(|| serde_json::from_str(&spec_str)))
    {
        ...
        Err(payload) => {
            ...
            unsafe {
                write_error(
                    err_out,
                    format!("panic in arena_mssql_playbook_verify: {msg}"),
                )
            };
    ...
    let outcome = catch_unwind(AssertUnwindSafe(|| -> Result<(), String> {
        ...
        let mssql_active = active
            .as_any()
            .downcast_ref::<ActivePlaybook>()
            .ok_or_else(|| "playbook handle is not an MSSQL playbook".to_string())?;

        let actual = inner
            .runtime_handle
            .block_on(async { mssql_active.verify(&parsed.query).await });
