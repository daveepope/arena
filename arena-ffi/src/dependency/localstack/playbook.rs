    ...
    F: FnOnce(&LocalstackDependency) -> R,
{
    ...
    let localstack = dep
        ...
        .ok_or_else(|| format!("dependency '{identifier}' is not a LocalstackDependency"))?;
    Ok(f(localstack))
}
...
pub extern "C" fn arena_localstack_playbook_open(
    ...
    if arena_handle.is_null() {
        unsafe {
            write_error(
                err_out,
                "arena_localstack_playbook_open: arena handle must not be null",
            )
        };
    ...
    if spec.is_null() {
        unsafe {
            write_error(
                err_out,
                "arena_localstack_playbook_open: spec must not be null",
            )
        };
        return std::ptr::null_mut();
    }
    ...
    let spec_str = match unsafe { c_str_to_string(spec) } {
        ...
        None => {
            unsafe {
                write_error(
                    err_out,
                    "arena_localstack_playbook_open: spec is not valid UTF-8",
                )
            };
            return std::ptr::null_mut();
        }
    ...
    let outcome = catch_unwind(AssertUnwindSafe(|| -> Result<PlaybookInner, String> {
        ...
        let playbook =
            with_localstack_dependency(arena_runtime, &parsed.dependency_identifier, |localstack| {
                localstack.playbook()
            })?;
    ...
    match outcome {
        ...
        Ok(Err(msg)) => {
            tracing::error!(error = %msg, op = "localstack_playbook_open", "playbook open failed");
...
pub extern "C" fn arena_localstack_playbook_close(
    ...
    match outcome {
        ...
        Err(payload) => {
            let msg = panic_message(&payload);
            tracing::error!(error = %msg, op = "localstack_playbook_close", "playbook close failed");
