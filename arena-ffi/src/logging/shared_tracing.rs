...
#[cfg(test)]
mod impl_allowlist_tests {
    use super::*;

    #[test]
    fn empty_allow_lists_admits_arena_matches_lifecycle_kv() {
        let dep_allow: Vec<String> = vec![];
        let comp_allow: Vec<String> = vec![];
        assert!(
            dispatcher_impl_allowlists_allows_delivery(
                "arena::matches",
                "[x] hello | dependency=\"arena-postgres-readings-api-postgres-ab\", phase=1",
                &dep_allow,
                &comp_allow,
            )
        );
    ...
    #[test]
    fn empty_allow_lists_blocks_arena_impl_dependency_logs() {
        let dep_allow: Vec<String> = vec![];
        let comp_allow: Vec<String> = vec![];
        assert!(
            !dispatcher_impl_allowlists_allows_delivery(
                "arena_postgres::postgres_dependency",
                "[x] m | dependency=\"arena-postgres-readings-api-postgres-xx\", phase=1",
                &dep_allow,
                &comp_allow,
            )
        );
    ...
    #[test]
    fn empty_allow_lists_admits_arena_container() {
        let dep_allow: Vec<String> = vec![];
        let comp_allow: Vec<String> = vec![];
        assert!(
            dispatcher_impl_allowlists_allows_delivery(
                "arena_container::network",
                "[arena_container::network] x | network=\"n\"",
                &dep_allow,
                &comp_allow,
            )
        );
    ...
    #[test]
    fn empty_allow_lists_blocks_arena_http_without_dep_fields_in_payload_tail() {
        let dep_allow: Vec<String> = vec![];
        let comp_allow: Vec<String> = vec![];
        assert!(
            !dispatcher_impl_allowlists_allows_delivery(
                "arena_http::http_dependency",
                "[arena_http::http_dependency] lone message",
                &dep_allow,
                &comp_allow,
            )
        );
    ...
    #[test]
    fn component_allow_alone_blocks_dependency_only_records() {
        let dep_allow: Vec<String> = vec![];
        let comp_allow = vec!["readings-api-web-app".to_owned()];
        assert!(
            !dispatcher_impl_allowlists_allows_delivery(
                "arena_postgres::postgres_dependency",
                "[x] m | dependency=\"arena-postgres-readings-api-postgres-xx\", phase=1",
    ...
    #[test]
    fn dependency_allow_list_blocks_unlisted_dependency_records() {
        let dep_allow = vec!["readings-api-oauth".to_owned()];
        let comp_allow: Vec<String> = vec![];
        assert!(
            !dispatcher_impl_allowlists_allows_delivery(
                "arena_postgres::postgres_dependency",
    ...
    #[test]
    fn dependency_allow_list_admits_matching_dependency_records() {
        let dep_allow = vec!["readings-api-postgres".to_owned()];
        let comp_allow: Vec<String> = vec![];
        assert!(
            dispatcher_impl_allowlists_allows_delivery(
                "arena_postgres::postgres_dependency",
    ...
    #[test]
    fn component_allow_list_admits_matching_component_records() {
        let dep_allow: Vec<String> = vec![];
        let comp_allow = vec!["readings-api-web-app".to_owned()];
        assert!(
            dispatcher_impl_allowlists_allows_delivery(
                "arena_executable_component::executable_component",
                "[x] m | component=\"arena-executable-component-readings-api-web-app-zz\", pid=3",
                &dep_allow,
                &comp_allow,
            )
        );
    ...
    #[test]
    fn dependency_allow_alone_blocks_component_only_records() {
        let dep_allow = vec!["readings-api-postgres".to_owned()];
        let comp_allow: Vec<String> = vec![];
        assert!(
            !dispatcher_impl_allowlists_allows_delivery(
    ...
    #[test]
    fn arena_ffi_crate_always_admitted_when_allow_lists_nonempty() {
        let dep_allow = vec!["nope-only-this".to_owned()];
        let comp_allow = vec!["nope-only-this".to_owned()];
        assert!(
            dispatcher_impl_allowlists_allows_delivery(
                "arena_ffi::logging",
                "[arena_ffi::logging] x",
    ...
    #[test]
    fn arena_ffi_always_admitted_when_allow_lists_nonempty() {
        let dep_allow = vec!["nope-only-this".to_owned()];
        let comp_allow = vec!["nope-only-this".to_owned()];
        assert!(
            dispatcher_impl_allowlists_allows_delivery(
                "arena::ffi",
                "[arena::ffi] arena log level set | arena_log_level=Debug",
                &dep_allow,
                &comp_allow,
            )
        );
