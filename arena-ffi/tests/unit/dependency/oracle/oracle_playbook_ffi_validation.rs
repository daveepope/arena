...
#[test]
fn oracle_playbook_verify_null_spec_returns_invalid_argument() {
    let mut err: *mut c_char = std::ptr::null_mut();
    let handle = 0x1 as *mut ArenaActivePlaybookHandle;
    let status = arena_oracle_playbook_verify(handle, std::ptr::null(), &mut err as *mut _);
    assert_eq!(status, ArenaStatus::InvalidArgument);
    assert!(err_text(err).contains("verify_spec must not be null"));
}

#[test]
fn oracle_playbook_verify_non_utf8_spec_returns_invalid_argument() {
    let invalid_utf8: &[u8] = &[0x66, 0x6f, 0xff, 0x00];
    let spec_ptr = invalid_utf8.as_ptr() as *const c_char;
    let mut err: *mut c_char = std::ptr::null_mut();
    let handle = 0x1 as *mut ArenaActivePlaybookHandle;
    let status = arena_oracle_playbook_verify(handle, spec_ptr, &mut err as *mut _);
    assert_eq!(status, ArenaStatus::InvalidArgument);
