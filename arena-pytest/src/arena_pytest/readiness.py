...
FFI-backed checks are serialized via :mod:`arena_pytest._ffi_readiness`; others may run
...
class HttpReadinessCheck:
    ...
    Serialization for the shared JSON contract lives in :mod:`arena_pytest._ffi_readiness`.
    ...
    async def is_ready(self, identifier: str, target: str, timeout_ms: int) -> None:
        from arena_pytest._http_wait import wait_for_http_ready
