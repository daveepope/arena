...
Release version is driven by the repo root **`VERSION`** file. On PRs, CI bumps it from `master` (patch by default; add **`semver:minor`** or **`semver:major`** labels for larger bumps). `Cargo.toml`, `MODULE.bazel`, and the Bazel wheel read that value via sync; `pyproject.toml` loads it dynamically from `../VERSION`. Keep `arena-pytest/LICENSE` aligned with the repository root `LICENSE`, then rebuild the wheel.
