...
- Tests that **start a running dependency** (container or in-process server) must declare **`tags = ["component_test"]`** on their **`rust_test`** / **`py_test`** / **`java_test`** target. PR CI runs the full suite (including `component_test`) on **Linux** (`ubuntu-24.04`) and on **Windows** (`windows-latest`). **`macos-15-intel`** and **`macos-latest`** run build and non-`component_test` tests only, with no container-runtime setup step on either. Coverage and hosts without a container runtime use **`--test_tag_filters=-component_test`**.
