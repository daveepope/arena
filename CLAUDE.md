...
- **Rust: no inline `#[cfg(test)] mod tests` blocks in `src/`.** Put unit tests in a separate file under the crate's `tests/` directory that mirrors the source module (e.g. `src/image.rs` → `tests/image_test.rs`), matching the sibling `*_test.rs` files already in each crate. Functions under test must be `pub` (or `pub(crate)` re-exported as needed) so the external test file can call them, and need a matching `rust_test` target in `BUILD`.
