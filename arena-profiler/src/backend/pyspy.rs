...
const PYSPY_RLOCATIONS: &[&str] =
    &["rules_rust~~crate~profiling_tools__py-spy-0.4.2/py-spy__bin"];
...
impl WrappingSampler for PySpySampler {
    fn wrap(
        ...
        let py_spy = resolve_binary(
            PYSPY_RLOCATIONS,
            PYSPY_PATH_FALLBACK,
            PYSPY_PATH_FALLBACK,
            INSTALL_HINT,
        )?;
        let folded_path = scratch_path("arena-profiler-pyspy", "folded");
