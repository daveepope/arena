...
fn prepare_cpu_profile_real_perf_wrapping_busy_loop_produces_html_report() {
    let output_path = std::env::temp_dir().join(format!(
        "arena-profiler-perf-component-test-{}.html",
        std::process::id()
    ));

    let request = LaunchRequest {
        program: "sh".into(),
        args: vec!["-c".to_string(), "i=0; while true; do i=$((i+1)); done".to_string()],
    };

    let prepared = prepare_cpu_profile(CpuProfilerBackend::Perf, request, &output_path)
        .expect("prepare perf profile");
