...
public final class ActiveHttpPlaybook implements AutoCloseable {
  private final OpenArena arena;
  private final String dependencyIdentifier;
  private final ArrayNode mappings;
  private Pointer handle;

  ActiveHttpPlaybook(OpenArena arena, String dependencyIdentifier, ArrayNode mappings) {
    this.arena = arena;
  ...
  public void begin() {
    ...
    try {
      ...
      handle = ArenaFfiPlaybooks.httpPlaybookBegin(arena, json);
    } catch (Exception e) {
  ...
  public void close() {
    ...
    try {
      ArenaFfiPlaybooks.httpPlaybookFinish(h);
  ...
  public void verify(String method, String urlPath, int expectedCount) {
    if (handle == null || Pointer.nativeValue(handle) == 0) {
      throw new IllegalStateException(
          "ActiveHttpPlaybook.verify requires begun playbook scope (begin() first)");
    }
    ObjectNode spec = ArenaJson.object();
    spec.put("dependency_identifier", dependencyIdentifier);
