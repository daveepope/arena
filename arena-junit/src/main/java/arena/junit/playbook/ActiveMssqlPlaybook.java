...
public class ActiveMssqlPlaybook extends ActivePlaybook {
  protected ActiveMssqlPlaybook(Pointer handle) {
    super(handle);
  }
  ...
  public void verify(String query, int expectedValue) {
    ...
    try {
      ArenaBindings.mssqlPlaybookVerify(handle(), ArenaJson.MAPPER.writeValueAsString(spec));
