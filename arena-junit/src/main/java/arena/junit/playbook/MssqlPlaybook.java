...
public final class MssqlPlaybook implements AutoCloseable {
  private final String dependencyIdentifier;
  private Pointer handle;

  MssqlPlaybook(String dependencyIdentifier) {
  ...
  public void open(OpenArena arena) {
    ...
    try {
      handle =
          ArenaFfiPlaybooks.mssqlPlaybookOpen(arena, ArenaJson.MAPPER.writeValueAsString(spec));
    } catch (Exception e) {
  ...
  public void close() {
    ...
    ArenaFfiPlaybooks.mssqlPlaybookClose(h);
  }

  public void verify(String query, int expectedValue) {
    if (handle == null || Pointer.nativeValue(handle) == 0) {
      throw new IllegalStateException("MssqlPlaybook.verify called outside of active playbook");
    }
    ObjectNode spec = ArenaJson.object();
    spec.put("dependency_identifier", dependencyIdentifier);
