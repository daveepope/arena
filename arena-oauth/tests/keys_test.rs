use arena_oauth::{IssuerConfig, OauthDependency};
use jsonwebtoken::{decode, Algorithm, DecodingKey, Validation};
use rsa::pkcs8::{DecodePrivateKey, EncodePublicKey, LineEnding};
use rsa::RsaPrivateKey;
use serde_json::{json, Value};

fn decoding_key_for(dep: &OauthDependency, index: usize) -> DecodingKey {
...
fn sign_claims_with_arbitrary_claims_produces_verifiable_token() {
    ...
    let token = dep.sign_claims(0, &claims).expect("sign claims");

    let mut validation = Validation::new(Algorithm::RS256);
    validation.set_issuer(&["https://issuer.example"]);
    let decoded = decode::<Value>(&token, &decoding_key_for(&dep, 0), &validation)
...
#[test]
fn sign_claims_unknown_issuer_index_returns_err() {
    let dep = OauthDependency::builder("keys-sign-claims-unknown-index")
        .with_http()
        .build();
    let claims = json!({ "sub": "test-subject" });
    let err = dep
        .sign_claims(1, &claims)
        .expect_err("issuer index 1 does not exist on a single-issuer dependency");
...
fn private_key_pkcs8_pem_roundtrips_through_from_pkcs8_pem() {
    let original = OauthDependency::builder("keys-pem-roundtrip-source")
        .with_http()
        .build();
    let pem = original.signing_key_pem(0).expect("signing key pem");
