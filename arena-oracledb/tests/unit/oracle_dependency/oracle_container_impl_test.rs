...
async fn exec_sql_wraps_sql_in_script_before_calling_run_sqlplus() {
    ...
    let result = oracle_container_impl::exec_sql(&fake, "user", "pass", "SELECT 1 FROM dual").await;

    assert_eq!(result, Ok("OUTPUT".to_string()));
    let (username, password, script) = fake.last_call.lock().expect("last_call lock").clone().unwrap();
...
async fn exec_sql_run_sqlplus_error_propagates_to_caller() {
    ...
    let result = oracle_container_impl::exec_sql(&fake, "user", "pass", "SELECT 1").await;
...
async fn exec_scalar_query_numeric_stdout_returns_parsed_i32() {
    ...
    let result =
        oracle_container_impl::exec_scalar_query(&fake, "user", "pass", "SELECT COUNT(*) FROM widgets").await;
...
async fn exec_scalar_query_non_numeric_stdout_returns_err() {
    ...
    let result = oracle_container_impl::exec_scalar_query(&fake, "user", "pass", "SELECT 1").await;
...
async fn exec_table_list_multiline_stdout_returns_parsed_names() {
    ...
    let result =
        oracle_container_impl::exec_table_list(&fake, "user", "pass", "SELECT TABLE_NAME FROM USER_TABLES").await;
